...
def export_onnx(
    ...
    if end2end:
        ...
        score0_sorted = torch.topk(score0_tensor, topK)

        topK_index = score0_sorted.indices[0][:topK]

        kpts0_topK = kpts0[..., topK_index, :]
        desc0_topK = desc0[..., topK_index, :]

        score1_tensor = torch.Tensor(scores1)
        score1_sorted = torch.topk(score1_tensor, topK)

        topK_index = score1_sorted.indices[0][:topK]
        kpts1_topK = kpts1[..., topK_index, :]
