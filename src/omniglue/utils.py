...
from lightglue_test.onnx_runner import load_image, viz2d
...
def visualize_matches(
    ...
    if not is_opencv_show:
        print(" >>>>> img0_path = {0} - img1_path = {1} <<<<<< ".format(image0_path, image1_path))
        ...
        viz2d.plot_matches(kp0, kp1,  lw=0.2)
        viz2d.add_text(0, f'Match:{len(kp0)}', pos=(0.2, 0.9), fs=20, color='yellow', lcolor='blue', lwidth=3, ha='center', va='center')
        ...
        for match in matches:
            ...
            if match_labels is None:
                ...
                if match_labels[match[0], match[1]]:
                    ...
                    color = (255, 0, 0)
            cv2.line(viz, pt0, pt1, color, line_width, lineType=cv2.LINE_AA)
